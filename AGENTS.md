...
- Preserve the returned `session_id` across follow-up calls and pass the
  returned `retrieval_id` as `parent_retrieval_id` when one retrieval leads to
  another or to `neurofs_expand`.
- After finishing a task that used those results, call `neurofs_feedback`
  once with that exact `retrieval_id`: rating `yes`/`no`/`partial`, the
  symbols/paths that actually helped, and any identifier that should have been
  retrieved but wasn't. Only name symbols you verified exist. This feeds
  `neurofs learn` (see docs/self_improvement.md).
